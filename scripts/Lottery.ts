...
async function checkState() {
  ...
  console.info(`The lottery is ${state ? "open" : "closed"}\n`);
  if (!state) return;
  const currentBlock = await ethers.provider.getBlock("latest");
  const currentBlockDate = new Date(currentBlock.timestamp * 1_000);
  const closingTime = await contract.closingTime();
  const closingTimeDate = new Date(closingTime.toNumber() * 1_000);
  console.info(
...
async function openBets(duration: string) {
  ...
  console.info(`Bets opened (${receipt.transactionHash})`);
}
...
async function buyTokens(index: string, amount: string) {
  ...
  const receipt = await tx.wait();
  console.info(`Tokens bought (${receipt.transactionHash})\n`);
}
